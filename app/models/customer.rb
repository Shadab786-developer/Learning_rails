class CustomEmailValidator < ActiveModel::Validator
  def validate(record)
    options[:fields].each do |field|
      ...
      if value.blank?
        record.errors.add field, "is requried ?"
      elsif value.include?("spam")
        record.errors.add field , "contains invalid word 'spam'"
      end
    end
...
class Customer < ApplicationRecord
  ...
  validates_with CustomEmailValidator, fields:[:customer_email,:customer_feedback]
